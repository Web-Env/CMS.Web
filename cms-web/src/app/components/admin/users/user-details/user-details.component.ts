...
import { 
    loadContentTimeTrackingsByUserId, 
    LOAD_CONTENT_TIME_TRACKINGS_BY_USER_ID_FAILURE, 
    LOAD_CONTENT_TIME_TRACKINGS_BY_USER_ID_SUCCESS } from "src/app/ngrx/actions/content-time-tracking/content-time-tracking.actions";
import { 
    loadUserById, 
    LOAD_USER_BY_ID_FAILURE, 
    LOAD_USER_BY_ID_SUCCESS } from "src/app/ngrx/actions/user/user.actions";
...
export class UserDetailsComponent implements OnDestroy, OnInit {
    ...
    loadDataFailedSubscription!: Subscription;
    ...
    ngOnInit(): void {
        ...
        if (this.urlHelperService.isRouteParamObjectValid(routeParams) &&
                ...
                .subscribe((data: any) => {
                    if (data !== undefined && data.type !== undefined) {
                        if (data.type === LOAD_USER_BY_ID_SUCCESS) {
                            ...
                            if (data.contentTimeTrackings !== null && data.contentTimeTrackings !== undefined) {
            ...
            this.loadDataSubscription = this.actions$
                .pipe(ofType(LOAD_USER_BY_ID_FAILURE, LOAD_CONTENT_TIME_TRACKINGS_BY_USER_ID_FAILURE))
                .subscribe((data: any) => {
                    if (data !== undefined && data.type !== undefined) {
                        if (data.type === LOAD_USER_BY_ID_FAILURE) {
                            this.userIsLoading = false;
                        }
                    }
                }
    ...
    public getData(routeParams: Params): void {
        this.userIsLoading = true;
        this.isContentTimeTrackingLoaded = false;
    ...
    ngOnDestroy(): void {
        ...
        this.loadDataFailedSubscription?.unsubscribe();
    }
